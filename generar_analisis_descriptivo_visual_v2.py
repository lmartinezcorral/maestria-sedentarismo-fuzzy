...
def cargar_datos_consolidados():
    ...
    df = df.dropna(subset=['Fecha']).sort_values(['Usuario', 'Fecha']).reset_index(drop=True)
    
    print(f"✅ Cargados: {len(df):,} registros, {df['Usuario'].nunique()} usuarios")
    return df
...
def cargar_datos_individuales():
    ...
    if not archivos:
        raise FileNotFoundError(f"No se encontraron archivos: {DB_USUARIOS_PATTERN}")
    ...
    df_consolidado = df_consolidado.dropna(subset=['Fecha']).sort_values(['Usuario', 'Fecha']).reset_index(drop=True)
    
    print(f"✅ Consolidados: {len(df_consolidado):,} registros, {df_consolidado['Usuario'].nunique()} usuarios")
...
def calcular_estadisticos_descriptivos(df, variables):
    ...
    for var in variables:
        ...
        if len(data) < 10:
            print(f"⚠️  Variable {var} con muy pocos datos ({len(data)}), omitiendo...")
            continue
...
def exportar_tabla_latex(df_tabla, output_file):
    ...
    latex_lines.append("\\caption{Estadísticos Descriptivos Actualizados (Datos Post-Limpieza)}")
...
def plot_violin_por_usuario(df, variables, output_dir):
    ...
    for idx, var in enumerate(variables):
        ...
        ax.set_ylabel(NOMBRES_AMIGABLES.get(var, var), fontsize=11, fontweight='bold')
        ax.set_title(NOMBRES_AMIGABLES.get(var, var), fontsize=12, fontweight='bold')
...
def plot_grouped_bar_medianas(df, variables, output_dir):
    ...
    for idx, var in enumerate(variables[:8]):  # Limitar a 8 vars para legibilidad
...
def plot_scatter_matrix(df, variables, output_dir):
    ...
    df_sample = df[vars_disponibles + ['Usuario']].dropna().sample(min(2000, len(df)), random_state=42)
    
    # Renombrar columnas
    df_sample_renamed = df_sample.rename(columns=NOMBRES_AMIGABLES)
    
    # Scatter matrix con colores por usuario
    g = sns.PairGrid(df_sample_renamed, hue='Usuario', palette=USER_COLORS, height=2.5, aspect=1)
    g.map_upper(sns.scatterplot, alpha=0.5, s=20)
    g.map_lower(sns.kdeplot, alpha=0.6)
    g.map_diag(sns.histplot, kde=True, alpha=0.6)
    g.add_legend(title='Usuario', bbox_to_anchor=(1.05, 0.5), loc='center left', fontsize=9)
...
def plot_time_series_ultimos_90_dias(df, variables, output_dir):
    ...
    for dias_offset in [0, 180, 365, 545]:  # Últimos 90d, hace 6m, hace 1a, hace 1.5a
        ...
        df_ventana = df[(df['Fecha'] >= fecha_inicio) & (df['Fecha'] <= fecha_fin)]
    ...
    df_90d = df[(df['Fecha'] >= fecha_min_90d) & (df['Fecha'] <= fecha_max_90d)].copy()
    
    print(f"  📅 Ventana seleccionada: {fecha_min_90d.date()} a {fecha_max_90d.date()} (n={len(df_90d):,})")
    ...
    for idx, var in enumerate(vars_disponibles):
        ...
        for user_idx, usuario in enumerate(sorted(df_90d['Usuario'].unique())):
            df_user = df_90d[df_90d['Usuario'] == usuario][['Fecha', var]].dropna().sort_values('Fecha')
            
            if len(df_user) < 5:
                continue
            
            # Suavizado rolling mean 7 días
            df_user['valor_suavizado'] = df_user[var].rolling(window=7, min_periods=1, center=True).mean()
    ...
    axes[-1].set_xlabel('Fecha (últimos 90 días)', fontsize=12, fontweight='bold')
...
def plot_histogramas_con_kde(df, variables, output_dir):
    ...
    for idx, var in enumerate(variables):
        ...
        if len(data) < 10:
            ax.text(0.5, 0.5, f'{var}\nDatos insuficientes', ha='center', va='center', fontsize=12)
            ax.set_xticks([])
            ax.set_yticks([])
            continue
        ...
        ax.axvline(mean_val, color='orange', linestyle='--', linewidth=2, label=f'Media: {mean_val:.1f}')
        ax.axvline(median_val, color='green', linestyle='--', linewidth=2, label=f'Mediana: {median_val:.1f}')
        
        ax.set_xlabel(NOMBRES_AMIGABLES.get(var, var), fontsize=11, fontweight='bold')
...
def main():
    ...
    try:
        ...
        print("⚠️  Archivo consolidado no encontrado, intentando con archivos individuales...")
        df_consolidado = cargar_datos_individuales()
    
    # Calcular estadísticos descriptivos
    print("\n📊 Calculando estadísticos descriptivos...")
    df_stats = calcular_estadisticos_descriptivos(df_consolidado, VARIABLES_CLAVE)
    ...
    plot_time_series_ultimos_90_dias(df_consolidado, VARIABLES_CLAVE, OUTPUT_DIR)
    ...
    print(f"🎨 Figuras generadas: {len(list(OUTPUT_DIR.glob('*.png')))} archivos PNG")
    print()
