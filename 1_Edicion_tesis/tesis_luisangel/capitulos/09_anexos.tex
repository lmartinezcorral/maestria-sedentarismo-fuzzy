...
Esta sección presenta el análisis comparativo exhaustivo de 17 investigaciones representativas sobre clasificación de comportamiento sedentario y actividad física mediante dispositivos \textit{wearables}. La tabla abarca diversas técnicas metodológicas (lógica difusa, machine learning, deep learning, métodos estadísticos), tipos de validación (destacando \textit{LOUO}/LOSO) y aplicaciones en contextos de vida libre. Para facilitar la lectura, la tabla se presenta en orientación horizontal (landscape).
...
Capitoli \textit{et al.}. (2025) \cite{Capitoli2025FuzzyXAI} & 
Diagnóstico clínico interpretable con lógica difusa probabilística & 
\textit{fuzzy} Logic (interpretable) & 
Datos clínicos hospitalarios & 
Clínico & 
Validación clínica & 
Interpretabilidad 100\% & 
Clínicos controlan proceso diagnóstico. \textit{fuzzy} = XAI \\

\midrule

Marashi-Hosseini \textit{et al.}. (2023) \cite{MarashiHosseini2023Dietary} & 
Sistema decisión dietética para pacientes múltiples condiciones crónicas & 
\textit{fuzzy} Logic \textit{Mamdani} (1144 reglas) & 
...
Bassani \textit{et al.}. (2025) \cite{Bassani2025DLHAR} & 
...
Salim \textit{et al.}. (2024) \cite{Salim2024STEPHEN} & 
Detección tiempo sedentario + bouts con Hidden Semi-Markov Model & 
Hidden Semi-Markov Model & 
\textit{wearables} consumo (muñeca) & 
--- & 
...
Migueles \textit{et al.}. (2022) \cite{Migueles2022GRANADA} & 
...
Lyons \textit{et al.}. (2024) \cite{Lyons2024StandHour} & 
Definición umbrales actividad que activan ``Stand Hour'' en \textit{Apple Watch} & 
Estudio observacional & 
\textit{Apple Watch} (energía activa + pasos) & 
--- & 
Análisis umbral & 
Umbrales Stand Hour & 
...
Ji \textit{et al.}. (2023) \cite{Ji2023Scratch} & 
...
Bienefeld \textit{et al.}. (2023) \cite{Bienefeld2023XAI} & 
...
Deng \textit{et al.}. (2023) \cite{Deng2023LharJBHI} & 
HAR ligero en \textit{wearables} con knowledge distillation & 
Deep Learning (ligero) & 
Sensores \textit{wearables} & 
--- & 
Validación experimental & 
\textit{Accuracy} + eficiencia & 
\textbf{IEEE JBHI} (revista objetivo). HAR eficiente \textit{wearables} \\
...
Marino \textit{et al.}. (2024) \cite{Marino2024ARIC} & 
Asociaciones PA + \textit{HRV} con función cognitiva + demencia (ARIC Neurocognitive Study) & 
Análisis cohorte longitudinal & 
ECG ambulatorio 2 semanas + acelerómetro & 
961 & 
Análisis regresión & 
Odds Ratio (OR) & 
Mayor PA + \textit{HRV} $\rightarrow$ mejor cognición. Relación no lineal PA-\textit{HRV}-salud cerebral \\
...
\(\mathcal{R}\) & Base de reglas (conjunto de 5 reglas \textit{Mamdani}) \\
\(w_r^{(j)}\) & Activación (firing strength) de regla \(R_r\) para observación \(j\) \\
\(\mathbf{w}^{(j)}\) & Vector de activaciones \(\in \mathbb{R}^5\) \\
\(T(a, b)\) & T-norm (operador AND \textit{fuzzy}): \(T(a,b) = \min(a,b)\) \\
\(y_r\) & \textit{output} crisp de la regla \(R_r\) \\
...
\(\mathcal{D}\) & \textit{dataset} completo (1,337 semanas, 10 usuarios) \\
...
\(F1^{(i)}\) & \textit{F1-Score} en fold \(i\) (usuario \(i\) como test) \\
\(\overline{F1}_{\text{LOOU}}\) & \textit{F1-Score} promedio \textit{LOOU} (10 folds) \\
\(\sigma_{F1}\) & Desviación estándar de \textit{F1-Score} entre folds \\
...
\(S\) & Silhouette Score (métrica de calidad \textit{clustering}) \\
\\
...
\(F1\) & \textit{F1-Score}: \(2 \cdot \text{Prec} \cdot \text{Rec} / (\text{Prec} + \text{Rec})\) \\
\(\text{MCC}\) & Matthews Correlation Coefficient \\
\(\text{Acc}\) & \textit{Accuracy} (exactitud): \((\text{TP}+\text{TN})/N\) \\
\\
