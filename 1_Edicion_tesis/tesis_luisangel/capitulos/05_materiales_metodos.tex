...
Esto motivó el diseño de variables derivadas que: (a) reduzcan redundancia informativa, y (b) normalicen por exposición temporal y metabolismo basal, lo que permite comparaciones equitativas entre usuarios con diferentes características antropométricas.
...
Se aplicó el algoritmo K-Means sobre el conjunto de 1,337 semanas válidas. Se utilizaron las ocho características derivadas (medianas e IQRs de las cuatro variables). La selección del número óptimo de conglomerados (K) se determinó mediante:
...
\textbf{Base de reglas:} Se establecieron 81 reglas del tipo SI-ENTONCES (3$^4$ combinaciones), las cuales incorporan conocimiento experto de fisiología del ejercicio. Ejemplo: ``SI Actividad\_rel es \textit{baja} Y Superávit\_cal es \textit{negativo} ENTONCES Índice\_AF es \textit{bajo}''.
