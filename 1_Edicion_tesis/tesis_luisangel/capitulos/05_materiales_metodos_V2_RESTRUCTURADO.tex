...
\begin{enumerate}
    ...
    \item \textbf{R2}: IF Actividad\_relativa = \textit{Baja} AND \textit{HRV-SDNN} = \textit{Alta} THEN Sedentarismo = \textit{Bajo} (baja actividad compensada por alta VFC $\to$ protección cardiovascular)
    \item \textbf{R3}: IF \textit{HRV-SDNN} = \textit{Baja} AND Delta\_cardiaco = \textit{Bajo} THEN Sedentarismo = \textit{Alto} (pobre salud cardiovascular $\to$ riesgo)
    \item \textbf{R4}: IF Actividad\_relativa = \textit{Media} AND \textit{HRV-SDNN} = \textit{Media} THEN Sedentarismo = \textit{Medio} (estado intermedio balanceado)
