...
def log(msg):
    ...
    msg_clean = msg.replace('✅', '[OK]').replace('❌', '[ERROR]').replace('📊', '[DATA]').replace('📈', '[GRAPH]')
    print(msg_clean)
...
def clustering_train(df_train):
    ...
    cluster_alto_original = cluster_means.idxmin()  # Cluster con menor actividad → Alto (ID ORIGINAL)
...
def fuzzy_fuzzify(df, mf_params, scalers):
    ...
    print(f"DEBUG fuzzy_fuzzify: df_norm columns = {[c for c in df_norm.columns if '_norm' in c]}")
    ...
    for feat in FEATURES_FUZZY:
        ...
        if feat_norm in df_norm.columns:
            print(f"DEBUG {feat}_norm: min={df_norm[feat_norm].min():.3f}, max={df_norm[feat_norm].max():.3f}, mean={df_norm[feat_norm].mean():.3f}")
    ...
    print(f"DEBUG fuzzy_fuzzify: df_memb shape = {df_memb.shape}, columns = {len(df_memb.columns)}")
    return df_memb
...
def fuzzy_inference(df_memb):
    ...
    print(f"DEBUG fuzzy_inference: df_memb columns = {df_memb.columns.tolist()}")
    ...
    # DEBUG
    print(f"DEBUG w1: min={w1.min():.3f}, max={w1.max():.3f}, mean={w1.mean():.3f}")
    print(f"DEBUG w2: min={w2.min():.3f}, max={w2.max():.3f}, mean={w2.mean():.3f}")
    ...
    # DEBUG
    print(f"DEBUG s_alto: min={s_alto.min():.3f}, max={s_alto.max():.3f}, mean={s_alto.mean():.3f}")
    print(f"DEBUG s_total: min={(s_bajo + s_medio + s_alto).min():.3f}, max={(s_bajo + s_medio + s_alto).max():.3f}")
...
def main():
    ...
    for test_user in usuarios:
        ...
        # 2. Calcular percentiles MF sobre datos NORMALIZADOS
        log("  [1b] Calculando percentiles MF sobre datos normalizados...")
