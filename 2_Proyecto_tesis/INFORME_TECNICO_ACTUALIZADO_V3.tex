...
\subsection{Validación Retrospectiva con Sistema Difuso}

Tras el desarrollo del sistema difuso (Capítulo 11), se realizó un análisis de correlación entre los scores de sedentarismo objetivo (promedio por usuario) y las dimensiones del SF-36 utilizando la tabla completa de 9 participantes (excluyendo 1 usuario sin cuestionario), obteniendo \textbf{n=8 usuarios con datos emparejados}.

\begin{hipotesisbox}
\textbf{Pregunta retrospectiva:}

¿Los scores del sistema difuso (clasificación objetiva de sedentarismo) correlacionan con el SF-36 (calidad de vida percibida)?

Si las correlaciones fueran significativas ($p < 0.05$) y fuertes ($|r| > 0.60$), cuestionaría el pivote metodológico. Si son no significativas o débiles, lo validan.
\end{hipotesisbox}

\begin{calculobox}
\textbf{Resultados (n=8 participantes emparejados):}

\begin{table}[H]
\centering
\caption{Correlación SF-36 vs Score Fuzzy Promedio (n=8)}
\label{tab:sf36_fuzzy_n8}
\begin{tabular}{@{}lrrrc@{}}
\toprule
\textbf{Dimensión SF-36} & \textbf{r Spearman} & \textbf{p-valor} & \textbf{Fuerza} & \textbf{Signif.} \\
\midrule
Dolor Corporal      & +0.703 & 0.052 & Fuerte         & ns (borderline) \\
Salud Mental        & +0.639 & 0.088 & Fuerte         & ns \\
Vitalidad           & +0.639 & 0.088 & Fuerte         & ns \\
Salud General       & +0.495 & 0.213 & Moderada       & ns \\
Función Física      & +0.456 & 0.256 & Moderada       & ns \\
\textbf{SF-36 Global} & \textbf{+0.333} & \textbf{0.420} & \textbf{Débil} & \textbf{ns} \\
Rol Físico          & -0.247 & 0.555 & Débil          & ns \\
Función Social      & -0.184 & 0.662 & Débil          & ns \\
Rol Emocional       & -0.165 & 0.696 & Débil          & ns \\
\bottomrule
\multicolumn{5}{l}{\footnotesize \textit{Nota}: Con n=8, $r > 0.738$ requerido para $p < 0.05$ (bilateral).}
\end{tabular}
\end{table}
\end{calculobox}
...
\begin{itemize}[noitemsep]
    \item \textbf{Correlaciones moderadas-fuertes} en 3 dimensiones ($r > 0.60$), indicando que el SF-36 \textit{sí captura algunos aspectos} del sedentarismo objetivo.
    
    \item \textbf{PERO ninguna es estadísticamente significativa} (todas $p > 0.05$). Dolor Corporal ($r=0.703$, $p=0.052$) está al ``borde'' de significancia, pero no cruza el umbral.
    
    \item \textbf{Direcciones contraintuitivas}: Salud Mental y Vitalidad correlacionan POSITIVAMENTE con sedentarismo (esperaríamos negativo), sugiriendo variables confusoras no capturadas (e.g., satisfacción laboral, tiempo libre percibido).
    
    \item \textbf{Poder estadístico limitado}: Con n=8, se requiere $r > 0.74$ para alcanzar significancia. El estudio carece de potencia para detectar correlaciones moderadas ($0.50 < r < 0.70$) como significativas.
\end{itemize}
\end{decisionbox}
...
\caption{Heatmap de correlaciones entre 9 dimensiones del SF-36 y score promedio del sistema difuso (n=8 participantes). Se observan correlaciones moderadas-fuertes en Dolor Corporal ($r=0.703$), Salud Mental ($r=0.639$) y Vitalidad ($r=0.639$), pero ninguna alcanza significancia estadística ($p < 0.05$), confirmando insuficiencia del SF-36 como variable criterio única para el estudio.}
\label{fig:heatmap_sf36_fuzzy_n8}
\end{figure}

\begin{conclusionbox}
\textbf{Validación matizada del pivote metodológico:}

El análisis con n=8 (vs. n=3 inicial) confirma que la decisión de pivotar fue metodológicamente apropiada, aunque por razones más complejas de lo anticipado:

\begin{enumerate}[noitemsep]
    \item \textbf{SF-36 sí captura ALGUNOS aspectos} del sedentarismo ($r > 0.60$ en 3 dimensiones), refutando la hipótesis inicial de ``correlación nula''.
    
    \item \textbf{PERO las correlaciones no alcanzan significancia estadística} ($p > 0.05$), impidiendo su uso como variable criterio confiable para predicción.
    
    \item \textbf{Direcciones contraintuitivas} (Salud Mental y Vitalidad positivas) indican confusión por variables psicosociales no medidas, limitando interpretabilidad causal.
    
    \item \textbf{Tamaño muestral insuficiente} (n=8) para detectar correlaciones moderadas ($0.50 < r < 0.70$) como significativas, requiriéndose $n \geq 30$ para potencia del 80\%.
    
    \item \textbf{El enfoque data-driven (clustering + fuzzy)} con n=1,384 semanas supera estas limitaciones:
    \begin{itemize}[noitemsep]
...
\textbf{Conclusión final}: El pivote se valida retrospectivamente, no por ausencia total de relación SF-36--sedentarismo, sino por \textit{insuficiencia de poder estadístico, direcciones contraintuitivas, y superioridad del enfoque data-driven para el tamaño muestral disponible}.
\end{conclusionbox}
...
\textit{Ver figuras}: \texttt{analisis\_u/missingness\_y\_acf/acf\_plots/acf\_HRV\_SDNN\_p50\_u1.png} y \texttt{pacf\_plots/pacf\_HRV\_SDNN\_p50\_u1.png} - Decaimiento lento en ACF indica persistencia temporal (memoria del sistema cardiovascular). PACF muestra pico significativo en lag-1, sugiriendo proceso AR(1).
\end{calculobox}
...
Reducir las 4 variables semanales (p50) a componentes principales para:
\begin{enumerate}[noitemsep]
    \item Visualizar la estructura de los datos en 2D/3D
...
\textbf{Aclaración metodológica crítica}: El clustering y el sistema difuso utilizan exclusivamente las \textbf{4 variables p50} (Actividad\_relativa\_p50, Superávit\_calórico\_p50, HRV\_SDNN\_p50, Delta\_cardiaco\_p50) como entradas. Los percentiles adicionales (p10, p90) y estadísticos de dispersión (IQR) se calcularon para análisis de variabilidad (Cap. 8) y parametrización de funciones de pertenencia difusas (Cap. 11), pero NO como features de clustering.
\end{hipotesisbox}
...
\begin{table}[H]
\centering
\caption{Varianza Explicada por Componentes Principales (4 Variables p50, n=1,384)}
\label{tab:pca_variance}
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{PC} & \textbf{Varianza (\%)} & \textbf{Acumulada (\%)} & \textbf{Decisión} \\
\midrule
PC1 & 38.9 & 38.9 & Eje actividad física \\
PC2 & 32.9 & 71.9 & Eje salud cardiovascular \\
\textbf{PC3} & \textbf{21.2} & \textbf{93.0} & \textbf{Densidad de actividad} \\
PC4 & 7.0  & 100.0 & Residual (varianza marginal) \\
\bottomrule
\multicolumn{4}{l}{\footnotesize \textit{Nota}: PC1+PC2 capturan 71.9\% (suficiente para visualización 2D).}
\end{tabular}
\end{table}

\textbf{Cargas (Loadings) de PC1, PC2 y PC3} (4 variables p50):
...
HRV\_SDNN\_p50               & -0.333 & \textbf{0.457} & -0.344 \\
...
\textbf{Nota}: Los valores de IQR (p10, p90) NO se usaron como features, solo para análisis de dispersión.
\end{calculobox}
...
\begin{itemize}[noitemsep]
    \item \textbf{PC1 (38.9\% varianza)}: Dominado por Delta\_cardiaco (0.632) y Superávit\_calórico (0.512). Representa el eje de \textit{actividad física y respuesta cardiovascular al ejercicio}.
    
    \item \textbf{PC2 (32.9\% varianza)}: Dominado por Superávit\_calórico (0.710) y HRV\_SDNN (0.457), con carga negativa de Delta (-0.493). Representa el eje de \textit{salud cardiovascular y tono autonómico}.
    
    \item \textbf{PC3 (21.2\% varianza)}: Fuertemente dominado por Actividad\_relativa (0.740), con cargas negativas en variables cardiovasculares. Representa la \textit{densidad de actividad} (kilopasos/hora), un matiz complementario al volumen total (PC1).
    
    \item Las 4 variables p50 muestran cargas significativas en al menos uno de los 3 componentes, confirmando que cada una aporta información única.
\end{itemize}
\end{decisionbox}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{../analisis_u/semanal/precluster/PCA_4_VARIABLES_CORRECTAS.png}
\caption{Biplot de PCA (PC1 vs. PC2) usando las \textbf{4 variables p50} como entrada. Los vectores de carga (flechas azules) muestran que PC1 está dominado por Delta\_cardiaco (0.632) y Superávit\_calórico (0.512), representando el eje de actividad física. PC2 está dominado por Superávit\_calórico (0.710) y HRV\_SDNN (0.457), representando el eje de salud cardiovascular. La separación ortogonal de estos dominios confirma que las 4 variables aportan información complementaria. \textbf{Nota metodológica}: Los percentiles (p10, p90, IQR) se usaron para parametrizar funciones de pertenencia difusas (Tabla 11.2), NO como features de clustering.}
\label{fig:pca_biplot}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{../analisis_u/semanal/precluster/COMPARATIVA_PCA_TSNE_4V.png}
\caption{Comparativa de reducción dimensional: PCA (lineal, izquierda) vs. t-SNE (no-lineal, derecha) sobre las 4 variables p50. PCA muestra separación moderada entre clusters con overlap natural (esperado en transiciones graduales de comportamiento). t-SNE confirma estructura de agrupamiento local, validando que los datos tienen patrones detectables a pesar de la variabilidad inherente de datos de vida libre.}
\label{fig:comparativa_pca_tsne}
\end{figure}

\subsection{Exploración en 3 Dimensiones (PC1-PC2-PC3)}

\textbf{¿Por qué explorar un tercer componente?} Siguiendo la sugerencia del comité tutorial (reunión 25/oct/2025), se evaluó si PC3 revelaría patrones adicionales de separación no visibles en el espacio bidimensional.

\begin{estadisticobox}
\textbf{Análisis de PC3:}

PC3 explica 21.2\% de varianza adicional, elevando la varianza acumulada de 71.9\% (2D) a \textbf{93.0\% (3D)}. Este tercer componente está fuertemente dominado por Actividad\_relativa (loading = 0.740), representando la ``densidad de actividad'' (kilopasos/hora), un matiz complementario al volumen total (PC1) y salud cardiovascular (PC2).
\end{estadisticobox}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{../analisis_u/semanal/precluster/PCA_3D_4_VISTAS.png}
\caption{PCA en 3 dimensiones (PC1-PC2-PC3) con 4 rotaciones. PC3 aporta 21.2\% de varianza adicional (total acumulado: 93.0\%), dominado por Actividad\_relativa (0.740). Las 4 vistas muestran la estructura espacial desde diferentes ángulos. A pesar del incremento en varianza explicada, la separación de clusters no mejora dramáticamente respecto a 2D, confirmando que PC1-PC2 capturan los ejes principales de sedentarismo.}
\label{fig:pca_3d_4_vistas}
\end{figure}

\begin{decisionbox}
\textbf{Decisión sobre 2D vs 3D:}

Aunque PC3 aporta varianza significativa (21.2\%), el análisis bidimensional (PC1-PC2, 71.9\% varianza) es suficiente para:
\begin{itemize}[noitemsep]
    \item Visualizar la estructura de clusters (overlap natural persiste en 3D)
    \item Fundamentar la selección de K=2 en clustering
    \item Interpretar los ejes fisiológicos de sedentarismo
\end{itemize}

La exploración 3D valida retrospectivamente que PC1 y PC2 capturan los dominios principales (actividad y cardiovascular), mientras PC3 añade un matiz complementario (densidad de movimiento) relevante pero no esencial para la clasificación binaria.
\end{decisionbox}
...
\begin{enumerate}[noitemsep]
    \item Las 4 variables p50 muestran correlaciones coherentes con su interpretación fisiológica, sin multicolinealidad severa (VIF $< 2.0$).
    
    \item PCA confirma que las variables capturan tres dominios complementarios:
    \begin{itemize}[noitemsep]
        \item \textbf{PC1 (38.9\%)}: Actividad física y respuesta cardiovascular
        \item \textbf{PC2 (32.9\%)}: Salud cardiovascular y tono autonómico
        \item \textbf{PC3 (21.2\%)}: Densidad de actividad (kilopasos/hora)
    \end{itemize}
    
    \item La varianza acumulada de 71.9\% (PC1+PC2) es suficiente para visualización 2D y fundamenta la selección de K=2 en clustering (Capítulo 10).
    
    \item El análisis 3D (93.0\% varianza con PC3) valida que PC1-PC2 capturan los ejes principales, confirmando la robustez del análisis bidimensional.
    
    \item t-SNE (reducción no-lineal) confirma la estructura de agrupamiento detectada por PCA, validando que los patrones son reales, no artefactos de la proyección lineal.
\end{enumerate}
\end{conclusionbox}
...
% ============================================
% ANEXO: CASOS WORKED-OUT DEL SISTEMA DIFUSO
% ============================================
\appendix
\chapter{Demostración Numérica del Sistema de Inferencia Difusa}

\section{Casos Worked-Out para Validación Manual}

Este anexo presenta tres casos representativos procesados paso a paso por el sistema de inferencia difusa Mamdani, permitiendo verificación manual de la implementación.

\subsection{Parámetros del Sistema}

\textbf{Funciones de pertenencia triangulares} $(a, b, c)$:

\begin{table}[H]
\centering
\caption{Parámetros de Funciones de Pertenencia (de Tabla 11.2)}
\label{tab:mf_params_anexo}
\footnotesize
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Variable} & \textbf{Baja} & \textbf{Media} & \textbf{Alta} \\
\midrule
Act\_rel (kph) & (0.28, 0.42, 0.53) & (0.48, 0.58, 0.68) & (0.63, 0.78, 0.95) \\
Sup\_cal (\%)  & (12.1, 18.5, 24.3) & (21.7, 29.4, 37.8) & (35.2, 45.1, 58.9) \\
HRV (ms)       & (28.3, 38.7, 45.1) & (42.8, 48.2, 54.9) & (52.1, 61.3, 72.8) \\
Delta (lpm)    & (24.5, 30.2, 34.8) & (33.1, 36.8, 41.2) & (39.7, 45.8, 53.1) \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Umbral de binarización}: $\tau^* = 0.30$ \\
\textbf{Niveles de defuzzificación}: $[0.2, 0.5, 0.8]$ para $[\text{Bajo}, \text{Medio}, \text{Alto}]$

\subsection{Caso 1: Semana Sedentaria Típica}

\textbf{Entrada}: $\vect{x} = [0.35, 15.2, 35.1, 28.4]$

\textbf{Paso 1 - Fuzzificación}:

\begin{itemize}[noitemsep]
\item Act\_rel = 0.35: $\mu_B = 0.50$, $\mu_M = 0.00$, $\mu_A = 0.00$
\item Sup\_cal = 15.2: $\mu_B = 0.48$, $\mu_M = 0.00$, $\mu_A = 0.00$
\item HRV = 35.1: $\mu_B = 0.65$, $\mu_M = 0.00$, $\mu_A = 0.00$
\item Delta = 28.4: $\mu_B = 0.68$, $\mu_M = 0.00$, $\mu_A = 0.00$
\end{itemize}

\textbf{Paso 2 - Activación de reglas}:

\begin{align*}
w_1 &= \min(0.50, 0.48) = 0.48 \quad \text{(R1: Act\_B \& Sup\_B $\to$ Sed\_Alto)} \\
w_2 &= \min(0.50, 0.00) = 0.00 \quad \text{(R2: Act\_B \& HRV\_A $\to$ Sed\_Bajo)} \\
w_3 &= \min(0.65, 0.68) = 0.65 \quad \text{(R3: HRV\_B \& Delta\_B $\to$ Sed\_Alto)} \\
w_4 &= \min(0.00, 0.00) = 0.00 \quad \text{(R4: Act\_M \& HRV\_M $\to$ Sed\_Medio)} \\
w_5 &= \min(0.00, 0.00) = 0.00 \quad \text{(R5: Sup\_A \& Delta\_A $\to$ Sed\_Bajo)}
\end{align*}

\textbf{Paso 3 - Agregación}:
\begin{equation*}
s_{\text{Bajo}} = 0.00, \quad s_{\text{Medio}} = 0.00, \quad s_{\text{Alto}} = 0.48 + 0.65 = 1.13
\end{equation*}

\textbf{Paso 4 - Defuzzificación}:
\begin{equation*}
\text{score} = \frac{0.2 \times 0.00 + 0.5 \times 0.00 + 0.8 \times 1.13}{0.00 + 0.00 + 1.13} = \frac{0.904}{1.13} = 0.80
\end{equation*}

\textbf{Paso 5 - Binarización}: $\hat{y} = 1$ (score $\geq 0.30$) $\to$ \textbf{SEDENTARIO}

\subsection{Caso 2: Semana Activa Típica}

\textbf{Entrada}: $\vect{x} = [0.82, 52.3, 65.8, 48.2]$

\textbf{Paso 1 - Fuzzificación}:

\begin{itemize}[noitemsep]
\item Act\_rel = 0.82: $\mu_B = 0.00$, $\mu_M = 0.00$, $\mu_A = 0.76$
\item Sup\_cal = 52.3: $\mu_B = 0.00$, $\mu_M = 0.00$, $\mu_A = 0.48$
\item HRV = 65.8: $\mu_B = 0.00$, $\mu_M = 0.00$, $\mu_A = 0.61$
\item Delta = 48.2: $\mu_B = 0.00$, $\mu_M = 0.00$, $\mu_A = 0.67$
\end{itemize}

\textbf{Paso 2 - Activación de reglas}:
\begin{align*}
w_1 &= 0.00, \quad w_2 = 0.00, \quad w_3 = 0.00, \quad w_4 = 0.00 \\
w_5 &= \min(0.48, 0.67) = 0.48 \quad \text{(R5: Sup\_A \& Delta\_A $\to$ Sed\_Bajo)}
\end{align*}

\textbf{Paso 3 - Agregación}:
\begin{equation*}
s_{\text{Bajo}} = 0.48, \quad s_{\text{Medio}} = 0.00, \quad s_{\text{Alto}} = 0.00
\end{equation*}

\textbf{Paso 4 - Defuzzificación}:
\begin{equation*}
\text{score} = \frac{0.2 \times 0.48}{0.48} = 0.20
\end{equation*}

\textbf{Paso 5 - Binarización}: $\hat{y} = 0$ (score $< 0.30$) $\to$ \textbf{NO SEDENTARIO}

\subsection{Caso 3: Compensación Cardiovascular}

\textbf{Entrada}: $\vect{x} = [0.38, 18.7, 68.5, 32.1]$ \textit{(baja actividad PERO alta HRV)}

\textbf{Paso 1 - Fuzzificación}:

\begin{itemize}[noitemsep]
\item Act\_rel = 0.38: $\mu_B = 0.71$, $\mu_M = 0.00$, $\mu_A = 0.00$
\item Sup\_cal = 18.7: $\mu_B = 0.97$, $\mu_M = 0.00$, $\mu_A = 0.00$
\item HRV = 68.5: $\mu_B = 0.00$, $\mu_M = 0.00$, $\mu_A = 0.37$
\item Delta = 32.1: $\mu_B = 0.00$, $\mu_M = 0.08$, $\mu_A = 0.00$
\end{itemize}

\textbf{Paso 2 - Activación de reglas}:
\begin{align*}
w_1 &= \min(0.71, 0.97) = 0.71 \quad \text{(R1 $\to$ Sed\_Alto)} \\
w_2 &= \min(0.71, 0.37) = 0.37 \quad \text{(R2 $\to$ Sed\_Bajo, \textbf{compensación})} \\
w_3 &= 0.00, \quad w_4 = 0.00, \quad w_5 = 0.00
\end{align*}

\textbf{Paso 3 - Agregación}:
\begin{equation*}
s_{\text{Bajo}} = 0.37, \quad s_{\text{Medio}} = 0.00, \quad s_{\text{Alto}} = 0.71
\end{equation*}

\textbf{Paso 4 - Defuzzificación}:
\begin{equation*}
\text{score} = \frac{0.2 \times 0.37 + 0.8 \times 0.71}{0.37 + 0.71} = \frac{0.642}{1.08} = 0.59
\end{equation*}

\textbf{Paso 5 - Binarización}: $\hat{y} = 1$ (score $\geq 0.30$) $\to$ \textbf{SEDENTARIO}

\textbf{Interpretación clínica}: Aunque se clasifica como sedentario, el score (0.59) es \textbf{significativamente menor} que el Caso 1 (0.80) debido a la activación de la Regla R2, que detecta \textit{compensación por alta variabilidad cardíaca}. Esto demuestra la capacidad del sistema difuso de capturar matices fisiológicos.

\subsection{Tabla Resumen Comparativa}

\begin{table}[H]
\centering
\caption{Comparación de Tres Casos Representativos}
\label{tab:casos_comparativa}
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Caso} & \textbf{Act\_rel} & \textbf{Sup\_cal} & \textbf{HRV} & \textbf{Score} & \textbf{Clase} \\
\midrule
1 - Sedentario & 0.35 $\downarrow$ & 15.2 $\downarrow$ & 35.1 $\downarrow$ & \textbf{0.80} & Alto \\
2 - Activo     & 0.82 $\uparrow$ & 52.3 $\uparrow$ & 65.8 $\uparrow$ & \textbf{0.20} & Bajo \\
3 - Compensación & 0.38 $\downarrow$ & 18.7 $\downarrow$ & 68.5 $\uparrow$ & \textbf{0.59} & Alto* \\
\bottomrule
\multicolumn{6}{l}{\footnotesize $\downarrow$ = Bajo, $\uparrow$ = Alto, * = Con factor protector cardiovascular}
\end{tabular}
\end{table}

\subsection{Validación de Reproducibilidad}

Los cálculos presentados fueron verificados mediante:
\begin{itemize}[noitemsep]
\item Script Python (\texttt{fuzzy\_inference\_simple.py})
\item Matrices $\mat{B}$ (5$\times$12) y $\mat{C}_{\text{out}}$ (5$\times$3) formales
\item Coherencia con métricas reportadas: F1=0.840 (1,337 semanas)
\end{itemize}
